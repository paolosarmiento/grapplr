...
const Sessions=memo(function Sessions({sessions,onDelete}){
  ...
  return(<div className="fade-in">
    ...
    {displayed.map(s=><SCard key={s.id} s={s} onDelete={onDelete}/>)}
  </div>);
});
...
function AICoach({sessions,profile}){
  ...
  return(<div className="fade-in">
    ...
    {/* AI Upgrade */}
    <div style={{...card,background:"linear-gradient(135deg,#0A1A0A,#0D1F0D)",borderColor:`${LIME}30`}}>
...
const Progress=memo(function Progress({sessions,competitions,setCompetitions,goals,setGoals,journal,profile,onAddComp,onAddGoal,onEditComp,showToast}){
  ...
  return(<div className="fade-in">
    ...
    {sub==="comp"&&<div>
      ...
      {competitions.map(c=>(<div key={c.id} style={card}><div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:8}}><div><p style={{margin:"0 0 2px",fontSize:15,fontWeight:500,color:"#fff"}}>{c.event}</p><p style={{margin:0,fontSize:12,color:"#555"}}>{c.date} · {c.location} · {c.gi?"Gi":"No-Gi"}</p></div><div style={{display:"flex",gap:10}}><span style={{fontSize:14,fontWeight:700,color:POS}}>{c.wins}W</span><span style={{fontSize:14,fontWeight:700,color:"#E24B4A"}}>{c.losses}L</span></div></div>
      {confirmDel===c.id?(<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderTop:"0.5px solid #2A2A2C"}}><span style={{fontSize:13,color:"#E24B4A",flex:1}}>Delete this event?</span><button onClick={()=>deleteComp(c.id)} style={{padding:"8px 16px",borderRadius:50,background:"#E24B4A",color:"#fff",border:"none",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>Delete</button><button onClick={()=>setConfirmDel(null)} style={{padding:"8px 16px",borderRadius:50,background:"#2C2C2E",color:"#8E8E93",border:"none",cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>Cancel</button></div>
      ):(<div style={{display:"flex",gap:8,borderTop:"0.5px solid #2A2A2C",paddingTop:10}}><button onClick={()=>onEditComp(c)} style={{flex:1,padding:"9px",borderRadius:50,background:"#2C2C2E",color:"#8E8E93",border:"none",cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>Edit</button><button onClick={()=>setConfirmDel(c.id)} style={{flex:1,padding:"9px",borderRadius:50,background:"#E24B4A18",color:"#E24B4A",border:"none",cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>Delete</button></div>)}</div>))}
    </div>}
    {sub==="goals"&&<div>
...
function Library({techniques,setTechniques,partners,setPartners,injuries,setInjuries,onAddPartner,onAddInjury,libSec,setLibSec}){
  ...
  if(libSec==="partners")return(<div className="fade-in"><BackBtn label="Training Partners" right={<button onClick={onAddPartner} style={{padding:"9px 18px",borderRadius:50,background:LIME,color:LIME_DK,border:"none",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",minHeight:44}}>+ Add</button>}/>
    ...
    {partners.map(p=>{const b=BELTS.find(x=>x.id===p.belt)||BELTS[0];return(<div key={p.id} style={card}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:46,height:46,borderRadius:"50%",background:b.color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:16,fontWeight:700,color:b.text}}>{p.name.charAt(0)}</span></div><div style={{flex:1}}><p style={{margin:"0 0 2px",fontSize:15,fontWeight:500,color:"#fff"}}>{p.name}</p><p style={{margin:0,fontSize:12,color:"#555"}}>{b.label} belt{p.gym?` · ${p.gym}`:""}</p></div><button onClick={()=>setPartners(prev=>prev.filter(x=>x.id!==p.id))} style={{width:44,height:44,background:"none",border:"none",cursor:"pointer",color:"#555",fontSize:14}}></button></div></div>);})}
  </div>);
...
function CompetitionModal({onClose,onSave,editItem}){
  const [f,setF]=useState(editItem||{date:todayISO(),event:"",location:"",weight:WEIGHTS[4],wins:0,losses:0,gi:true,notes:""});
  const sv=useCallback((k,v)=>setF(p=>({...p,[k]:v})),[]);
  return(<BottomSheet onClose={onClose} title={editItem?"Edit competition":"Add competition"}>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}><div><Lbl>Date</Lbl><input type="date" value={f.date} onChange={e=>sv("date",e.target.value)}/></div><div><Lbl>Format</Lbl><div style={{display:"flex",gap:8,marginTop:2}}><Pill active={f.gi} onClick={()=>sv("gi",true)}>Gi</Pill><Pill active={!f.gi} onClick={()=>sv("gi",false)}>No-Gi</Pill></div></div></div>
    ...
    <div style={{marginBottom:12}}><Lbl>Location</Lbl><input value={f.location} onChange={e=>sv("location",e.target.value)} placeholder="e.g. Manila"/></div>
    <div style={{marginBottom:12}}><Lbl>Weight class</Lbl><select value={f.weight} onChange={e=>sv("weight",e.target.value)}>{WEIGHTS.map(w=><option key={w}>{w}</option>)}</select></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}><Stepper label="Wins" value={f.wins} onChange={v=>sv("wins",Math.min(20,v))}/><Stepper label="Losses" value={f.losses} onChange={v=>sv("losses",Math.min(20,v))}/></div>
    <div style={{marginBottom:20}}><Lbl>Notes</Lbl><textarea value={f.notes} onChange={e=>sv("notes",e.target.value)} placeholder="How did it go?"/></div>
...
function PartnerModal({onClose,onSave}){
  const [f,setF]=useState({name:"",belt:"blue",gym:"",notes:""});
  const sv=useCallback((k,v)=>setF(p=>({...p,[k]:v})),[]);
  return(<BottomSheet onClose={onClose} title="Add training partner"><div style={{marginBottom:12}}><Lbl>Name <span style={{color:"#E24B4A"}}>*</span></Lbl><input value={f.name} onChange={e=>sv("name",e.target.value)} placeholder="Partner name"/></div><div style={{marginBottom:16}}><Lbl>Belt</Lbl><BeltPicker value={f.belt} onChange={v=>sv("belt",v)}/></div><div style={{marginBottom:12}}><Lbl>Gym</Lbl><input value={f.gym} onChange={e=>sv("gym",e.target.value)} placeholder="e.g. Flow Studio"/></div><div style={{marginBottom:20}}><Lbl>Notes</Lbl><textarea value={f.notes} onChange={e=>sv("notes",e.target.value)} placeholder="Training style, strengths, game..."/></div><PBtn onClick={()=>f.name.trim()&&onSave(f)} disabled={!f.name.trim()}>Add partner</PBtn></BottomSheet>);
}
...
export default function App(){
  ...
  if(!profile.onboarded)return(<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",background:"#000",minHeight:"100vh"}}><GS/><Onboarding onDone={p=>setProfile(p)}/></div>);

  const beltObj=BELTS.find(b=>b.id===profile.belt)||BELTS[1];
  const tabTitle={sessions:"Train",journal:"Journal",progress:"Progress",library:"Moves"};

  return(
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",maxWidth:680,margin:"0 auto",background:"#000",minHeight:"100vh"}}>
      <GS/>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",position:"sticky",top:0,background:"rgba(0,0,0,0.92)",backdropFilter:"blur(12px)",zIndex:100,borderBottom:"0.5px solid #1A1A1A"}}>
        <img src={LOGO} alt="Grapplr" style={{height:26,objectFit:"contain"}}/>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{padding:"16px 20px 100px"}}>
        ...
        {tab==="library"&&<Library techniques={techniques} setTechniques={setTechniques} partners={partners} setPartners={setPartners} injuries={injuries} setInjuries={setInjuries} onAddPartner={()=>setModal("partner")} onAddInjury={()=>setModal("injury")} libSec={libSec} setLibSec={setLibSec}/>}
      </div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:680,background:TAB_BG,display:"flex",alignItems:"flex-end",borderTop:"0.5px solid #1A1A1A",paddingBottom:8,paddingTop:4,zIndex:50}}>
        ...
        <div style={{flex:1,display:"flex",justifyContent:"center",alignItems:"flex-end",paddingBottom:2}}>
          <button onClick={()=>setModal("quicklog")} className="glow-btn" style={{width:58,height:58,borderRadius:"50%",background:LIME,border:`4px solid ${TAB_BG}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transform:"translateY(-16px)",fontSize:28,color:LIME_DK,fontWeight:800}} aria-label="Quick log">+</button>
        </div>
