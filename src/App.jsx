...
function calcChallengeProgress(sessions,journal,challenge){
  ...
  if(challenge.type==="duration")return{current:Math.max(0,...wkSessions.map(s=>s.duration||0)),target:challenge.target};
...
const Dashboard=memo(function Dashboard({sessions,journal,profile,onLog,onQuickLog,onJournal}){
  ...
  const weekStart=new Date();weekStart.setDate(weekStart.getDate()-weekStart.getDay());weekStart.setHours(0,0,0,0);
  const wkSessions=sessions.filter(s=>new Date(s.date)>=weekStart);
  const weeklyDots=useMemo(()=>{const days=["S","M","T","W","T","F","S"];return[...Array(7)].map((_,i)=>{const d=new Date(Date.now()-(6-i)*864e5);const key=d.toISOString().slice(0,10);const active=sessions.some(s=>s.date===key);return{day:days[d.getDay()],active};});},[sessions]);
...
//  SESSIONS 
const Sessions=memo(function Sessions({sessions,onDelete}){
  ...
  return(<div className="fade-in">
    ...
    {sel&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",flexDirection:"column",justifyContent:"flex-end"}} onClick={()=>setSel(null)}><div style={{background:"#1C1C1E",borderRadius:"24px 24px 0 0",maxHeight:"85vh",overflowY:"auto",padding:"20px 22px 44px"}} onClick={e=>e.stopPropagation()}><div style={{width:36,height:5,borderRadius:3,background:"#3A3A3C",margin:"0 auto 16px"}}/><h3 style={{margin:"0 0 16px",fontSize:19,fontWeight:700,color:"#fff"}}>Session details</h3><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}><span style={{padding:"6px 14px",borderRadius:50,background:"#2C2C2E",fontSize:13,color:"#fff"}}>{sel.date}</span><span style={{padding:"6px 14px",borderRadius:50,background:"#2C2C2E",fontSize:13,color:"#fff"}}>{sel.type}</span><span style={{padding:"6px 14px",borderRadius:50,background:"#2C2C2E",fontSize:13,color:"#fff"}}>{sel.duration} min</span>{sel.mood&&<span style={{padding:"6px 14px",borderRadius:50,background:MOOD_C[sel.mood]+"20",fontSize:13,color:MOOD_C[sel.mood],fontWeight:600}}>{sel.mood}</span>}</div>{sel.partner&&<div style={{marginBottom:12}}><p style={{margin:"0 0 4px",fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}}>Partner</p><p style={{margin:0,fontSize:15,color:"#fff"}}>{sel.partner}</p></div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}><div style={{background:"#2C2C2E",borderRadius:14,padding:"12px",textAlign:"center"}}><p style={{margin:0,fontSize:22,fontWeight:800,color:"#1D9E75"}}>{sel.taps_given||0}</p><p style={{margin:"4px 0 0",fontSize:11,color:"#555"}}>Subs landed</p></div><div style={{background:"#2C2C2E",borderRadius:14,padding:"12px",textAlign:"center"}}><p style={{margin:0,fontSize:22,fontWeight:800,color:"#E24B4A"}}>{sel.taps_received||0}</p><p style={{margin:"4px 0 0",fontSize:11,color:"#555"}}>Times tapped</p></div></div>{sel.notes&&<div style={{marginBottom:12}}><p style={{margin:"0 0 4px",fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}}>Notes</p><p style={{margin:0,fontSize:14,color:"#fff",lineHeight:1.6}}>{sel.notes}</p></div>}{sel.techniques?.length>0&&<div style={{marginBottom:12}}><p style={{margin:"0 0 8px",fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}}>Techniques</p><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{sel.techniques.map(t=><span key={t} style={{padding:"4px 12px",borderRadius:50,background:"#2C2C2E",fontSize:12,color:"#fff"}}>{t}</span>)}</div></div>}{sel.rounds?.length>0&&<div style={{marginBottom:12}}><p style={{margin:"0 0 8px",fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}}>Rounds</p>{sel.rounds.map((r,i)=><div key={r.id} style={{background:"#2C2C2E",borderRadius:12,padding:"10px 14px",marginBottom:6,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:13,color:"#fff"}}>Round {i+1}{r.partner?` · ${r.partner}`:""}</span><span style={{fontSize:12,fontWeight:600,color:r.result==="win"?"#1D9E75":r.result==="loss"?"#E24B4A":"#555"}}>{r.result}</span></div>)}</div>}<button onClick={()=>{onDelete(sel.id);setSel(null);}} style={{width:"100%",padding:"14px",borderRadius:14,background:"#E24B4A18",color:"#E24B4A",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:600,fontSize:14,marginTop:8}}>Delete session</button></div></div>}
    {displayed.map(s=><div key={s.id} onClick={()=>setSel(s)} style={{cursor:"pointer"}}><SCard s={s} onDelete={onDelete}/></div>)}
  </div>);
});
...
function AICoach({sessions,profile}){
  ...
  const getAICoaching=async()=>{
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:500,messages:[{role:"user",content:`You are an elite BJJ coach analyzing a student's training data. Be direct, specific, and actionable. No fluff. Student: ${JSON.stringify(summary)}. Give 3 specific, personalized coaching insights in 2-3 sentences each. Focus on their actual patterns, not generic advice. Format as plain text with each insight starting with an emoji.`}]})});
  ...
  return(<div className="fade-in">
    ...
    {/* AI Upgrade */}
    <div style={{display:"none",borderColor:`${LIME}30`}}>
...
const Progress=memo(function Progress({sessions,competitions,setCompetitions,goals,setGoals,journal,profile,onAddComp,onAddGoal,onEditComp,showToast}){
  ...
  const pbs=useMemo(()=>calcPersonalBests(sessions),[sessions]);
  const toggleGoal=useCallback(id=>{setGoals(p=>p.map(x=>x.id===id?{...x,done:true}:x));showToast(" Goal achieved! Keep pushing.");},[setGoals,showToast]);
  ...
  return(<div className="fade-in">
    ...
    {sub==="comp"&&<div>
      ...
      {competitions.map(comp=>{const medals=getMedals(comp);return(<div key={comp.id} style={{...card,cursor:"pointer"}} onClick={()=>setSelComp(comp)}><div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:8}}><div style={{flex:1}}><p style={{margin:"0 0 2px",fontSize:15,fontWeight:500,color:"#fff"}}>{comp.event}</p><p style={{margin:0,fontSize:12,color:"#555"}}>{comp.date}{comp.location?" · "+comp.location:""}</p></div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:14,fontWeight:700,color:POS}}>{comp.wins}W</span><span style={{fontSize:14,fontWeight:700,color:"#E24B4A"}}>{comp.losses}L</span></div></div>{medals.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>{medals.map((m,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:4,background:"#2C2C2E",borderRadius:50,padding:"3px 10px 3px 6px"}}><div style={{width:12,height:12,borderRadius:"50%",background:MEDAL_COLORS[m.medal],flexShrink:0}}/><span style={{fontSize:11,color:"#8E8E93"}}>{m.label}</span></div>)}</div>}{comp.photo&&<img src={comp.photo} style={{width:"100%",borderRadius:10,maxHeight:120,objectFit:"cover",marginBottom:8}}/>}
      {confirmDel===comp.id?(<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderTop:"0.5px solid #2A2A2C"}}><span style={{fontSize:13,color:"#E24B4A",flex:1}}>Delete this event?</span><button onClick={e=>{e.stopPropagation();deleteComp(comp.id);}} style={{padding:"8px 16px",borderRadius:50,background:"#E24B4A",color:"#fff",border:"none",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>Delete</button><button onClick={e=>{e.stopPropagation();setConfirmDel(null);}} style={{padding:"8px 16px",borderRadius:50,background:"#2C2C2E",color:"#8E8E93",border:"none",cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>Cancel</button></div>
    ...
    {goals.filter(g=>g.done).length>0&&<><SH>Completed</SH>{goals.filter(g=>g.done).map(g=>(<div key={g.id} style={{...card,opacity:0.5}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:44,height:44,borderRadius:"50%",background:LIME,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:LIME_DK,fontSize:18}}></div><p style={{margin:0,fontSize:14,textDecoration:"line-through",color:"#636366",flex:1}}>{g.title}</p><button onClick={()=>setGoals(p=>p.filter(x=>x.id!==g.id))} style={{width:36,height:36,borderRadius:"50%",background:"#E24B4A18",border:"none",cursor:"pointer",color:"#E24B4A",fontSize:13}}>✕</button></div></div>))}</>}
    </div>}
  </div>);
});
...
function Library({techniques,setTechniques,partners,setPartners,injuries,setInjuries,sessions=[],onAddPartner,onAddInjury,libSec,setLibSec}){
  ...
  const BackBtn=({label,right})=>(<div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}><button onClick={()=>setLibSec(null)} style={{width:44,height:44,borderRadius:"50%",background:"#1C1C1E",border:"none",cursor:"pointer",color:"#fff",fontSize:18}}>←</button><h2 style={{margin:0,fontSize:18,fontWeight:700,flex:1,color:"#fff"}}>{label}</h2>{right}</div>);

  if(libSec==="techniques")return(<div className="fade-in"><BackBtn label="Technique Library"/>
    ...
    {addOpen&&<BottomSheet onClose={()=>setAddOpen(false)} title="Add technique"><div style={{marginBottom:12}}><Lbl>Name</Lbl><input value={addName} onChange={e=>setAddName(e.target.value)} placeholder="e.g. Leg Lock"/></div><div style={{marginBottom:20}}><Lbl>Category</Lbl><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{["Submissions","Guard","Takedowns","Escapes","Sweeps","Other"].map(x=><Pill key={x} active={addCat===x} onClick={()=>setAddCat(x)} s={{fontSize:12}}>{x}</Pill>)}</div></div><PBtn onClick={()=>{if(addName.trim()){setTechniques(p=>[...p,{id:Date.now(),name:addName.trim(),cat:addCat,skill:1}]);setAddName("");setAddOpen(false);}}} disabled={!addName.trim()}>Add technique</PBtn></BottomSheet>}
  ...
  if(libSec==="partners")return(<div className="fade-in"><BackBtn label="Training Partners" right={<button onClick={onAddPartner} style={{padding:"9px 18px",borderRadius:50,background:LIME,color:LIME_DK,border:"none",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",minHeight:44}}>+ Add</button>}/>
  ...
  if(libSec==="injuries")return(<div className="fade-in"><BackBtn label="Injury Tracker" right={<button onClick={onAddInjury} style={{padding:"9px 18px",borderRadius:50,background:LIME,color:LIME_DK,border:"none",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",minHeight:44}}>+ Log injury</button>}/>
...
function PostSessionPrompt({onClose,onSave,sessionDate}){
  ...
  return(<BottomSheet onClose={onClose} title="Quick debrief">
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      <PBtn onClick={()=>onSave({...f,date:sessionDate||todayISO(),id:Date.now()})}>Save debrief</PBtn>
    </div>
  </BottomSheet>);
}
...
function SessionModal({onClose,onSave,techniques}){
  ...
  const addRound=()=>setF(p=>({...p,rounds:[...p.rounds,{id:Date.now(),partner:"",result:"neutral",notes:""}]}));
...
function CompetitionModal({onClose,onSave,editItem}){
  ...
  const sv=useCallback((k,v)=>setF(p=>({...p,[k]:v})),[]);
  const handlePhoto=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{const img=new Image();img.onload=()=>{const canvas=document.createElement("canvas");const max=800;let w=img.width,h=img.height;if(w>max){h=h*max/w;w=max;}canvas.width=w;canvas.height=h;canvas.getContext("2d").drawImage(img,0,0,w,h);sv("photo",canvas.toDataURL("image/jpeg",0.7));};img.src=ev.target.result;};reader.readAsDataURL(file);};
...
function PartnerDetailModal({partner,sessions,onClose,onSave,onDelete}){
  ...
  const sb=BELTS.find(x=>x.id===partner.belt)||BELTS[0];
  const pSess=sessions.filter(s=>s.partner&&s.partner.toLowerCase()===partner.name.toLowerCase());
  const lastT=pSess.length>0?pSess[0].date:null;
  ...
  return(<BottomSheet onClose={onClose} title={partner.name}><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,paddingBottom:20,borderBottom:"0.5px solid #2A2A2C"}}><div style={{width:56,height:56,borderRadius:"50%",background:sb.color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 0 16px ${sb.color}50`}}><span style={{fontSize:22,fontWeight:700,color:sb.text}}>{partner.name.charAt(0)}</span></div><div><p style={{margin:"0 0 2px",fontSize:16,fontWeight:700,color:"#fff"}}>{partner.name}</p><p style={{margin:0,fontSize:13,color:"#555"}}>{sb.label} belt{partner.gym?` · ${partner.gym}`:""}</p></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:20}}>{[{l:"Sessions",v:pSess.length,c:"#fff"},{l:"Last trained",v:lastT||"—",c:"#fff"},{l:"Avg subs/session",v:avgSubs,c:POS},{l:"Avg tapped/session",v:avgTapped,c:"#E24B4A"}].map(m=><div key={m.l} style={{background:"#2C2C2E",borderRadius:14,padding:"12px",textAlign:"center"}}><p style={{margin:0,fontSize:m.l==="Last trained"?12:20,fontWeight:700,color:m.c}}>{m.v}</p><p style={{margin:"4px 0 0",fontSize:10,color:"#555"}}>{m.l}</p></div>)}</div><div style={{marginBottom:12}}><Lbl>Name</Lbl><input value={en.name} onChange={e=>setEn(p=>({...p,name:e.target.value}))} placeholder="Partner name"/></div><div style={{marginBottom:16}}><Lbl>Belt</Lbl><BeltPicker value={en.belt} onChange={v=>setEn(p=>({...p,belt:v}))}/></div><div style={{marginBottom:12}}><Lbl>Gym</Lbl><input value={en.gym} onChange={e=>setEn(p=>({...p,gym:e.target.value}))} placeholder="e.g. Flow Studio"/></div><div style={{marginBottom:12}}><Lbl c={POS}>Strengths</Lbl><textarea value={en.strengths} onChange={e=>setEn(p=>({...p,strengths:e.target.value}))} placeholder="What are they dangerous at?" style={{minHeight:60}}/></div><div style={{marginBottom:12}}><Lbl c="#E24B4A">Weaknesses</Lbl><textarea value={en.weaknesses} onChange={e=>setEn(p=>({...p,weaknesses:e.target.value}))} placeholder="Where can you attack?" style={{minHeight:60}}/></div><div style={{marginBottom:12}}><Lbl>Game style</Lbl><input value={en.gameStyle} onChange={e=>setEn(p=>({...p,gameStyle:e.target.value}))} placeholder="e.g. Guard Player, Wrestler, Submission Hunter"/></div><div style={{marginBottom:20}}><Lbl>Notes</Lbl><textarea value={en.notes} onChange={e=>setEn(p=>({...p,notes:e.target.value}))} placeholder="Anything else to remember" style={{minHeight:60}}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><button onClick={onDelete} style={{padding:"14px",borderRadius:14,background:"#E24B4A18",color:"#E24B4A",border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:600,fontSize:14}}>Delete</button><PBtn onClick={()=>onSave(en)}>Save changes</PBtn></div></BottomSheet>);
}
...
export default function App(){
  ...
  const addJournal=useCallback(e=>{setJournal(p=>[{...e,id:Date.now()},...p]);setModal(null);setJournalInitDate(null);showToast("Journal entry saved ");},[showToast]);
  const addGoal=useCallback(g=>{setGoals(p=>[{...g,id:Date.now(),done:false},...p]);setModal(null);showToast("Goal added");},[showToast]);
  const addPartner=useCallback(p=>{setPartners(prev=>[{...p,id:Date.now()},...prev]);setModal(null);showToast("Partner added");},[showToast]);
  const addInjury=useCallback(i=>{setInjuries(prev=>[{...i,id:Date.now()},...prev]);setModal(null);showToast("Injury logged");},[showToast]);
  const saveComp=useCallback(c=>{if(editComp){setCompetitions(p=>p.map(x=>x.id===editComp.id?{...c,id:editComp.id}:x));showToast("Competition updated ");}else{setCompetitions(p=>[{...c,id:Date.now()},...p]);showToast("Competition saved ");}setEditComp(null);setModal(null);},[editComp,showToast]);
  const handleEditComp=useCallback(c=>{setEditComp(c);setModal("comp");},[]);
  const handleTabClick=useCallback((id)=>{setTab(id);if(id!=="library")setLibSec(null);},[]);

  if(authLoading)return(<div style={{minHeight:"100vh",background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}><img src={LOGO} alt="Grapplr" style={{width:"60%",maxWidth:200,objectFit:"contain",opacity:0.8}}/><p style={{color:"#555",fontSize:13,margin:0}}>Loading…</p></div>);
  ...
  return(
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif",maxWidth:680,margin:"0 auto",background:"#000",minHeight:"100vh",overflowX:"clip",width:"100%"}}>
      ...
      <div style={{padding:"16px 20px 100px"}}>
        ...
        {tab==="sessions"&&<Sessions sessions={sessions} onDelete={handleDeleteSession}/>}
